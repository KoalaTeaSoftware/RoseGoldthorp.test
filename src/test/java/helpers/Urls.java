    ...
    public static String interpretURL(String in) {
        ...
        if (matcher.find()) {
            try {
                ...
                writeToHtmlReport("Unable to supply a default scheme, or host");
            }
        }
        // if we have got here, there has been a problem (e.g. unable to match, or get defaults (if needed))
        writeToHtmlReport("Unable to make a good URL out of >" + in + "<");
        writeToHtmlReport("scheme: " + matcher.group(1));
        writeToHtmlReport("scheme's magic letters: " + matcher.group(2));
        writeToHtmlReport("host: " + matcher.group(3));
        writeToHtmlReport("the rest: " + matcher.group(4));
        return null;
    }
}
